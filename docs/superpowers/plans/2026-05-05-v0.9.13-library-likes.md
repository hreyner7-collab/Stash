...
Read the existing `InnerTubeClient.kt`. Find the existing public methods (`browse`, `search`, `player`, etc.). Add a new method following the same pattern, near the action-specific methods:
...
suspend fun likeVideo(videoId: String): Boolean {
    ...
    return runCatching {
        val outcome = executeRequestWithStatus(
            action = "like/like",
            body = payload,
            variant = ClientVariant.WEB_REMIX,
        )
        outcome.isSuccess
    }.getOrElse { e ->
...
(Verify the actual signature of `executeRequestWithStatus` in this file before pasting — the survey reported it's at line 507-601 with `internal` visibility. Method name might differ; the principle is to call into the existing internal request executor with `action = "like/like"` and the body shape `{"target": {"videoId": "..."}}`.)

If `executeRequestWithStatus` returns a different shape, adapt the success check. Goal: `likeVideo(videoId)` returns `Boolean` indicating success.
