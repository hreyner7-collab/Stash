...
| 2 | **New** `KennyyHealthProbe` | Background loop: on `start()` probe Kennyy once immediately; then probe with one cheap, short-timeout call on a **two-speed cadence** — a fast **ramp-down** interval (~2s) while probes are failing but health still reads healthy (so a dead Kennyy converges to unhealthy in ~4s, before the first play), and the slow **recovery** interval (~45s) once unhealthy (don't hammer a dead proxy). Idle entirely once a probe succeeds while healthy. Record each outcome into `KennyyHealthMonitor`. Sole caller of Kennyy while unhealthy → its successes flip health back to healthy. **The loop MUST survive every iteration** — see "Probe-loop resilience (critical)" below. |
